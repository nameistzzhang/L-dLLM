...
def default_image_transform(resolution=512, fill_color=(255, 255, 255)):
...
def encode_image_to_tokens(vq_model, image_pil, tokenizer, device, resolution=512, transform=None):
    if transform is None:
        transform = default_image_transform(resolution)
...
def worker(pretrained_model, rank, prompts, orig_idx, data_idx, image_paths, seq_dict, step_dict, batch_size, config):
    ...
    is_mmu = (config.model_base == "mmada" and config.dataset.data_type == "mmu")
    if is_mmu:
        assert MAGVITv2 is not None, "MAGVITv2 is not installed or not in PYTHONPATH."
        vq_model = MAGVITv2.from_pretrained(config.vq_model_path).to(device).eval()
        vq_model.requires_grad_(False)

        def left_pad_batch(tensors, pad_id):
            max_len = max(t.size(0) for t in tensors)
            out = torch.full((len(tensors), max_len), pad_id, dtype=torch.long, device=device)
            for i, t in enumerate(tensors):
                out[i, -t.size(0):] = t.to(device)
            return out
    ...
    for start in tqdm(range(0, len(prompts), batch_size),
        ...
        batch_didx    = data_idx[start:start+batch_size] if data_idx is not None else None
        ...
        if not is_mmu:
            ...
            for j, p in enumerate(batch_prompts):
                ...
                if did not in local_img_code_cache:
                    ...
                    img_codes = encode_image_to_tokens(
                        vq_model, img, tokenizer_gpu, device,
                        resolution=getattr(config, "image_resolution", 512)
                    ).detach().cpu()
                    local_img_code_cache[did] = img_codes
                img_codes = local_img_code_cache[did].to(device)

                chat_ids = tokenizer_gpu([p], add_special_tokens=False)["input_ids"][0]
                chat_ids = torch.tensor(chat_ids, dtype=torch.long, device=device)

                mmu = torch.tensor([MMU_ID, SOI_ID], dtype=torch.long, device=device)
                eoi = torch.tensor([EOI_ID], dtype=torch.long, device=device)

                full = torch.cat([mmu, img_codes, eoi, chat_ids], dim=0)
                mmu_inputs.append(full)

            pad_id = tokenizer_gpu.eos_token_id
            input_ids = left_pad_batch(mmu_inputs, pad_id=pad_id)
...
if __name__ == "__main__":
    ...
    if config.model_base == "mmada" and config.dataset.data_type == "mmu":
        image_paths = []
        for i in range(num):
